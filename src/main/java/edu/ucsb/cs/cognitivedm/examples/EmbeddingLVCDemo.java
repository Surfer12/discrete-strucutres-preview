...
public class EmbeddingLVCDemo {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("🧠 EMBEDDING-ENHANCED LEXICAL VIABILITY COMPONENT DEMO");
        System.out.println("   Cognitive-Inspired Mathematical Expression Processing");
        System.out.println(SEPARATOR);
        ...
        System.out.println("   Embedding-enhanced LVC shows significant improvements in");
        System.out.println("   term recall accuracy, cognitive alignment, and educational");
        System.out.println("   adaptability for mathematical expression processing.");
        System.out.println(SEPARATOR);
    }
    ...
    private static void demonstrateBasicEmbeddingSimilarity() {
        ...
        String[] setTheoryTerms = {"union", "intersection", "complement", "difference", "cartesian_product"};
        ...
        for (int i = 0; i < setTheoryTerms.length; i++) {
            for (int j = i + 1; j < setTheoryTerms.length; j++) {
                double similarity = embeddingService.calculateEmbeddingSimilarity(
                    setTheoryTerms[i], setTheoryTerms[j]
                );
                System.out.printf("   %s ↔ %s: %.3f\n",
                    setTheoryTerms[i], setTheoryTerms[j], similarity);
            }
        }
        ...
        for (int i = 0; i < notationTerms.length; i++) {
            for (int j = i + 1; j < notationTerms.length; j++) {
                double similarity = embeddingService.calculateEmbeddingSimilarity(
                    notationTerms[i], notationTerms[j]
                );
                System.out.printf("   %s ↔ %s: %.3f\n",
                    notationTerms[i], notationTerms[j], similarity);
            }
        }
        ...
        symbolMap.forEach((verbal, symbolic) -> {
            double similarity = embeddingService.calculateEmbeddingSimilarity(verbal, symbolic);
            System.out.printf("   %s ↔ %s: %.3f\n", verbal, symbolic, similarity);
        });
    }
    ...
    private static void demonstrateCognitiveAdaptation() {
        ...
        highAttentionResults.forEach(result -> 
            System.out.printf("     %s: %.3f\n", result.getTerm(), result.getSimilarity()));
    ...
    private static void demonstrateTermRecallOptimization() {
        ...
        try {
            AttentionRecognitionFramework framework = new AttentionRecognitionFramework();
            MathExpression expr = new MathExpression("{1,2,3} ∪ {4,5,6}", framework);

            // Test LVC viability calculation
            CognitiveState highAttentionState = new CognitiveState(0.9, 0.8, 0.2);
            CognitiveState lowAttentionState = new CognitiveState(0.3, 0.5, 0.8);

            System.out.println("🔍 Viability scores for different cognitive states:");
            
            // Get LVC from the expression
            MathExpression.LexicalViabilityComponent lvc = expr.getLexicalNetwork()
                .new LexicalViabilityComponent(expr.getLexicalNetwork(), expr.embeddingService);

            // Calculate viability for different terms
            String[] testTerms = {"union", "intersection", "roster", "symbolic"};
            
            System.out.println("   High attention state:");
            for (String term : testTerms) {
                try {
                    double viability = lvc.calculateViability(term, highAttentionState, Collections.emptyList());
                    System.out.printf("     %s: %.3f\n", term, viability);
                } catch (Exception e) {
            ...
            for (String term : testTerms) {
                try {
                    double viability = lvc.calculateViability(term, lowAttentionState, Collections.emptyList());
                    System.out.printf("     %s: %.3f\n", term, viability);
                } catch (Exception e) {
                    System.out.printf("     %s: calculation error\n", term);
                }
            ...
            String mostViableHigh = lvc.getMostViableTerm("{1,2} ∪ {3,4}", highAttentionState);
            String mostViableLow = lvc.getMostViableTerm("{1,2} ∪ {3,4}", lowAttentionState);
            
            System.out.printf("   High attention: %s\n", mostViableHigh);
            System.out.printf("   Low attention: %s\n", mostViableLow);

        } catch (Exception e) {
            System.out.println("   Error in term recall optimization demo: " + e.getMessage());
        }
    }
    ...
    private static void demonstrateΨOptimizationWithEmbeddings() {
        ...
        try {
            ...
            for (String exprStr : testExpressions) {
                try {
                    MathExpression expr = new MathExpression(exprStr, framework);
                    ...
                    System.out.printf("   Expression: %s - Error: %s\n", exprStr, e.getMessage());
                }
            }

        } catch (Exception e) {
            System.out.println("   Error in Ψ(x) optimization demo: " + e.getMessage());
        }
    }
    ...
    private static void demonstrateFeedbackLearning() {
        ...
        MathEmbeddingService.EmbeddingStats initialStats = embeddingService.getEmbeddingStats();
        System.out.printf("   Initial avg adaptive weight: %.3f\n", initialStats.getAvgAdaptiveWeight());
        
        // Simulate positive feedback
        System.out.println("\n   Applying positive feedback...");
        for (int i = 0; i < 3; i++) {
            embeddingService.updateEmbeddingFromFeedback(testTerm, context, 0.8, 0.3, true);
        }
        
        MathEmbeddingService.EmbeddingStats positiveStats = embeddingService.getEmbeddingStats();
        System.out.printf("   After positive feedback: %.3f\n", positiveStats.getAvgAdaptiveWeight());
        
        // Simulate negative feedback
        System.out.println("\n   Applying negative feedback...");
        for (int i = 0; i < 2; i++) {
            embeddingService.updateEmbeddingFromFeedback(testTerm, context, 0.8, 0.3, false);
        }
        
        MathEmbeddingService.EmbeddingStats negativeStats = embeddingService.getEmbeddingStats();
        System.out.printf("   After negative feedback: %.3f\n", negativeStats.getAvgAdaptiveWeight());
        
        // Show adaptation
        System.out.println("\n📊 Cognitive alignment adaptation:");
        System.out.printf("   Initial attention weight: %.3f\n", initialStats.getAttentionWeight());
        
        embeddingService.updateCognitiveAlignment(0.9, 0.8); // High load scenario
        MathEmbeddingService.EmbeddingStats adaptedStats = embeddingService.getEmbeddingStats();
        System.out.printf("   After high load adaptation: %.3f\n", adaptedStats.getAttentionWeight());
    }
    ...
    private static void demonstrateMultiScaleIntegration() {
        ...
        try {
            ...
            MathExpression expr = new MathExpression(complexExpression, framework);

            System.out.printf("🔍 Analyzing complex expression: %s\n", complexExpression);
            
            // Process with multi-scale framework
            CompletableFuture<MathExpression> future = expr.processWithCognitiveFramework();
            ...
            System.out.printf("   Processing state attention: %.3f\n", 
                processedExpr.getProcessingState().getAttention());
            System.out.printf("   Processing state cognitive load: %.3f\n", 
                processedExpr.getProcessingState().getCognitiveLoad());
            
            // Show cognitive tags
            System.out.println("\n🏷️  Cognitive tags:");
            processedExpr.getCognitiveTags().forEach((tag, value) -> 
                System.out.printf("     %s: %.3f\n", tag, value));
            
            // Show embedding statistics
            MathExpression.LexicalViabilityComponent lvc = processedExpr.getLexicalNetwork()
                .new LexicalViabilityComponent(process
